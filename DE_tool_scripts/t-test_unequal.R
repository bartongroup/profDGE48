...
# This script is essentially a copy of t-test_equal.R, but setting the 
# 'var.equal' parameter in t.test() as false.
#
# Assumes log normal distribution
# 
# Run with: Rscript t-test_unequal.R geneExprFile groupsFile outFile
#
# Author: Chris Cole
###############################################################################
...
if (length(args) < 3) {
	if (args[1] == '--version') {
		cat(sprintf("t-test_unequal.R version\t%s\n", Version))
...
for (i in 1:nrow(geneCounts.sub)) {
	#print(i)
	r=t.test(log(cond1[i,]),log(cond2[i,]),var.equal=F)
	res.t[i] = r$p.value
}
